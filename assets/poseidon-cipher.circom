...
template PoseidonEncrypt(length) {
    // the length of the plaintext input
    // must be padded to a multiple of 3
    var paddedLength = length;
    while (paddedLength % 3 != 0) {
        paddedLength++;
    }
    ...
    signal input nonce;
    signal input key[2];
    signal output ciphertext[paddedLength + 1];
    ...
    signal paddedMessage[paddedLength];
    ...
    var n = paddedLength \ 3;

    // Initialize Poseidon state components
    ...
    states[0].inputs[3] <== nonce + (length << 128);

    // Encrypt message blocks
    for (var i = 0; i < n; i++) {
        // Generate ciphertext for current block
        for (var j = 0; j < 3; j++) {
            ciphertext[i * 3 + j] <== paddedMessage[i * 3 + j] + states[i].out[j + 1];
        }

        // Prepare next state if not last block
        if (i < n) {
            states[i + 1] = PoseidonPerm(4);
            states[i + 1].inputs[0] <== states[i].out[0];
            
            // Set inputs from current ciphertext block
            for (var j = 0; j < 3; j++) {
    ...
    // Set final authentication tag (???)
    ciphertext[paddedLength] <== states[n].out[1];
}